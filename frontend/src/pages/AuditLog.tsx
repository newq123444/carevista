    ...
    <div>
      ...
      <div className="card" style={{ marginBottom:16 }}>
        <div className="card-body" style={{ display:'flex', gap:10, flexWrap:'wrap', alignItems:'flex-end' }}>
          <div className="form-group" style={{ marginBottom:0, flex:'1 1 180px' }}><label className="form-label">{tr('Search')}</label><input className="form-input" placeholder="User, details…" value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}}/></div>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
            <thead>
              <tr style={{ borderBottom:'2px solid var(--border)', background:'var(--surface-2)' }}>
                ...
                <th style={{ padding:'10px 14px', textAlign:'left', fontWeight:600 }}>{tr('Details')}</th>
                <th style={{ padding:'10px 14px', textAlign:'left', fontWeight:600 }}>IP</th>
              </tr>
            </thead>
            <tbody>
              {isLoading?(<tr><td colSpan={6} style={{ padding:40, textAlign:'center', color:'var(--text-muted)' }}>{tr('Loading…')}</td></tr>)
