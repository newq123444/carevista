  ...
  return (
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={() => calculateMutation.mutate({})} disabled={calculateMutation.isPending} style={{ padding: '10px 20px', background: '#0d9488', color: '#fff', border: 'none', borderRadius: 8, fontWeight: 600, cursor: 'pointer' }}>
        ...
        <>
          ...
          {metrics.length === 0 && (
            <div style={{ padding: 32, textAlign: 'center', background: '#f9fafb', borderRadius: 12, border: '1px dashed #d1d5db', color: '#6b7280', marginBottom: 24 }}>
              No benchmark data yet. Use <strong>{tr('Calculate KPIs')}</strong> to generate this home's metrics from live care data.
            </div>
          )}
                  ...
                  {kpi.percentile_rank != null && (
                    <div style={{ marginTop: 8 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                        <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>{tr('Percentile Rank')}</span>
          ...
          <div style={{ marginTop: 24, display: 'flex', gap: 16, alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <span style={{ fontSize: '0.8rem', color: '#6b7280' }}>{tr('Above Average')}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 12, height: 12, borderRadius: '50%', background: '#ca8a04' }} />
              <span style={{ fontSize: '0.8rem', color: '#6b7280' }}>{tr('Near Average')}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 12, height: 12, borderRadius: '50%', background: '#dc2626' }} />
              <span style={{ fontSize: '0.8rem', color: '#6b7280' }}>{tr('Below Average')}</span>
            </div>
          </div>
        </>
      )}
