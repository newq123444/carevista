      ...
      <div className="card">
        <div className="table-container">
          <table className="table">
            <thead>
              <tr><th>Invoice #</th><th>{tr('Resident')}</th><th>Period</th><th>{tr('Amount')}</th><th>Payer</th><th>{tr('Status')}</th><th>Due Date</th><th>{tr('Actions')}</th></tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                (invoices as Invoice[]).map(inv => (
                  <tr key={inv.id} style={{ background: inv.status === 'overdue' ? 'rgba(196,54,39,.04)' : undefined }}>
                    ...
                    <td>
                      <button className="btn btn-secondary btn-sm" onClick={() => setUpdating(inv)}>{tr('Update')}</button>
                    </td>
                  </tr>
                ))
              )}
      ...
      <div className="modal modal-lg">
        ...
        <form onSubmit={submit}>
          <div className="modal-body">
            ...
            <div className="form-group"><label className="form-label">{tr('Notes')}</label><textarea className="form-input" rows={2} value={form.notes} onChange={e => set('notes', e.target.value)} /></div>
          </div>
      ...
      <div className="modal modal-sm">
        ...
        <form onSubmit={submit}>
          <div className="modal-body">
            ...
            <div className="form-group"><label className="form-label">{tr('Status')}</label>
              <select className="form-input" value={form.status} onChange={e => setForm((f: any) => ({ ...f, status: e.target.value }))}>
                <option value="draft">Draft</option><option value="sent">Sent</option><option value="paid">{tr('Paid')}</option><option value="overdue">{tr('Overdue')}</option><option value="cancelled">{tr('Cancelled')}</option>
              </select>
            </div>
            {form.status === 'paid' && (
            ...
            <div className="form-group"><label className="form-label">{tr('Notes')}</label><textarea className="form-input" rows={3} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} /></div>
          </div>
          <div className="modal-footer">
            <button type="button" className="btn btn-secondary" onClick={onClose}>{tr('Cancel')}</button>
