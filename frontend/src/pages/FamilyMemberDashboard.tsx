  ...
  return (
    <div style={shell}>
      <header style={{ background: teal, color: '#fff', padding: '16px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => logout()} style={{ background: 'rgba(255,255,255,.15)', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 13, cursor: 'pointer' }}>{tr('Sign out')}</button>
      </header>
...
function Updates({ rid }: { rid?: string }) {
  ...
  if (isLoading) return <Card><span style={{ color: '#94a3b8' }}>{tr('Loading…')}</span></Card>;
      ...
      {wb[0] && (
        <Card title="How they're doing">
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <Pill label={tr('Mood')} value={wb[0].mood} />
            <Pill label={tr('Appetite')} value={wb[0].appetite} />
            <Pill label={tr('Energy')} value={wb[0].energy} />
            <Pill label={tr('Social')} value={wb[0].social} />
          </div>
  ...
  return (
    <Card title="Messages with the care team">
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        ...
        <button onClick={submit} disabled={!text.trim() || send.isPending} style={{ padding: '10px 16px', borderRadius: 10, background: 'var(--primary, #0f766e)', color: '#fff', border: 'none', fontWeight: 600, cursor: 'pointer' }}>{tr('Send')}</button>
      </div>
    </Card>
  );
}
...
function Care({ rid }: { rid?: string }) {
  ...
  if (isLoading || !data) return <Card><span style={{ color: '#94a3b8' }}>{tr('Loading…')}</span></Card>;
  const c = data.careThisWeek;
  return (
    <>
      <Card title={tr('Wellbeing')}>
