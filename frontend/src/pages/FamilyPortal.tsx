        ...
        {tabs.map(t => (
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 20,
              border: `1px solid ${tab === t.key ? '#0d9488' : 'var(--border)'}`,
        ...
        <div className="card-body" style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 12 }}>
            <div style={{ textAlign: 'center', padding: '8px 16px', borderRadius: 10, background: '#f0fdf4', border: '1px solid #bbf7d0' }}>
              ...
              <div style={{ fontSize: 11, color: '#15803d' }}>{tr('Wellbeing')}</div>
            </div>
          ...
          <div className="card-body">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              {summaryData.mood_summary && (
                <div>
                  <h4 style={{ margin: '0 0 6px', fontSize: 13, color: '#0d9488' }}>😊 Mood</h4>
    ...
    <div>
      {unread > 0 && (
        <div style={{ padding: '10px 16px', borderRadius: 10, background: '#eff6ff', border: '1px solid #bfdbfe', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
          <span>💬</span>
          <span style={{ color: '#0d9488', fontWeight: 600 }}>{unread} unread message{unread !== 1 ? 's' : ''} from families</span>
          <button
            onClick={() => messages.filter(m => !m.read_at && m.direction === 'inbound').forEach(m => markRead.mutate(m.id))}
            style={{ marginLeft: 'auto', fontSize: 12, color: '#0d9488', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}
          >
            Mark all read
          </button>
        </div>
          ...
          {(['inbox', 'sent'] as const).map(t => (
            <button
              ...
              style={{
                padding: '7px 16px',
                borderRadius: 20,
                border: `1px solid ${msgTab === t ? '#0d9488' : 'var(--border)'}`,
                background: msgTab === t ? '#eff6ff' : 'white',
                color: msgTab === t ? '#0d9488' : 'var(--text-muted)',
                cursor: 'pointer',
                fontSize: 13,
      ...
      {isLoading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((m: any) => {
            ...
            return (
              <div key={m.id} className="card" style={{ borderLeft: `4px solid ${isUnread ? '#0d9488' : 'transparent'}`, cursor: isUnread ? 'pointer' : 'default' }}
                      ...
                      {replyTo === m.id ? (
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button onClick={() => replyText.trim() && replyMutation.mutate({ id: m.id, body: replyText.trim() }, { onSuccess: () => { setReplyText(''); setReplyTo(null); } })}
                            ...
                            style={{ padding: '7px 16px', borderRadius: 8, border: 'none', background: '#0d9488', color: '#fff', cursor: 'pointer', fontSize: 12, fontWeight: 700 }}>{tr('Send')}</button>
                          <button onClick={() => { setReplyTo(null); setReplyText(''); }} style={{ padding: '7px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface-2, #f3f4f6)', cursor: 'pointer', fontSize: 12 }}>{tr('Cancel')}</button>
                        </div>
                      ) : (
                        <button onClick={() => { setReplyTo(m.id); setReplyText(''); }} style={{ padding: '5px 14px', borderRadius: 8, border: '1px solid #bfdbfe', background: '#eff6ff', color: '#0d9488', cursor: 'pointer', fontSize: 12, fontWeight: 600 }}>{tr('Reply')}</button>
                      )}
                    </div>
                  )}
                </div>
