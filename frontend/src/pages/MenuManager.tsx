        ...
        <SectionCard key={m} title={`${mealIcon[m]} ${m.charAt(0).toUpperCase() + m.slice(1)}`}>
          ...
          {byMeal(m).map((o: any) => (
            <div key={o.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderTop: '1px solid var(--border)' }}>
              ...
              <button onClick={() => openEdit(o)} style={{ padding: '5px 11px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--surface-2)', fontSize: 12, cursor: 'pointer', color: 'var(--text-primary)' }}>{tr('Edit')}</button>
              <button onClick={() => { if (confirm(`Remove "${o.name}"?`)) remove.mutate(o.id); }} style={{ padding: '5px 11px', borderRadius: 7, border: '1px solid var(--danger)', background: 'var(--danger-light)', color: 'var(--danger)', fontSize: 12, cursor: 'pointer' }}>{tr('Remove')}</button>
            </div>
          ))}
        </SectionCard>
      ))}
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 9000, background: 'rgba(15,23,42,.55)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', overflowY: 'auto', padding: 32 }}>
          <div style={{ width: '100%', maxWidth: 460, background: 'var(--surface)', borderRadius: 14, padding: 22 }}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              <div><label style={label}>{tr('Meal')}</label><select style={input} value={form.meal_type} onChange={e => set('meal_type', e.target.value)}>{MEALS.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
              <div><label style={label}>{tr('Texture')}</label><select style={input} value={form.texture} onChange={e => set('texture', e.target.value)}>{TEXTURES.map(t => <option key={t} value={t}>{t}</option>)}</select></div>
            </div>
